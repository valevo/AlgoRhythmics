...
class ChordGenerator(DataGenerator):
    def __init__(self, path, save_conversion_params=True,
        ...
        if save_conversion_params:
            ...
            self.save_conversion_params(filename="ChordGenerator.conversion_params")
    ...
    def generate_data(self):
        ...
        for ins_chords, ins_melody in zip(chord_iter, melody_iter):
            for (chords, meta), (melodies, meta_mel) in zip(ins_chords, ins_melody):
                ...
                for bar_chords, bar_melodies, meta_bar in zip(chords, melodies_mat, meta):
                    if bar_chords:
                        for single_chord in bar_chords:
                            #yield single_chord, bar_melodies, meta_bar
                            yield self.label_d[single_chord], bar_melodies, meta_bar
